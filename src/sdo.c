...
void SDOTimeoutAlarm(CO_Data *d, UNS32 id)
{
    ...
    UNS8 nodeId = READ_UNS8(d->objdict, offset+d->transfers[line].CliServNbr, 3);

    MSG_ERR("SDO timeout. SDO response not received.");
    MSG_ERR("server node id : %d, index: 0x%X, subIndex: 0x%X", 
        nodeId, d->transfers[line].index, d->transfers[line].subIndex);
    /* Reset timer handler */
...
void StopSDO_TIMER(CO_Data *d, UNS8 line)
{
    if (d)
    {
        MSG_DEBUG("StopSDO_TIMER for line : %d", line);
        d->transfers[line].timer = DelAlarm(d->transfers[line].timer);
    }
}
...
void StartSDO_TIMER(CO_Data *d, UNS8 line)
{
    if (d)
    {
        MSG_DEBUG("StartSDO_TIMER for line : %d", line);
        d->transfers[line].timer = SetAlarm(d, line, &SDOTimeoutAlarm, (TIMEVAL)MS_TO_TIMEVAL(SDO_TIMEOUT_MS), 0);
    }
}
...
void RestartSDO_TIMER(CO_Data *d, UNS8 line)
{
    if (d)
    {
        MSG_DEBUG("restartSDO_TIMER for line : %d", line);
        if(d->transfers[line].timer != TIMER_NONE)
        { 
            StopSDO_TIMER(d, line);
            StartSDO_TIMER(d, line);
        }
    }
}
...
UNS32 SDOLineToObjDict (CO_Data *d, UNS8 line)
{
    UNS32 size;
    UNS32 errorCode;
    MSG_DEBUG("Enter in SDOLineToObjDict (line %d)", line);
    ...
    if (size > SDO_MAX_LENGTH_TRANSFER) // Use dynamic buffer, need to allocate memory
    {
        errorCode = setODentry(d, d->transfers[line].index, d->transfers[line].subIndex,
                (void *) d->transfers[line].dynamicData, &size, 1);
    }
    else // Use stack array, do not need dynamic allocation
    {
        errorCode = setODentry(d, d->transfers[line].index, d->transfers[line].subIndex,
    ...
    MSG_DEBUG("exit of SDLineToObjDict (line %d)", line);
    return 0;
...
UNS32 objDictToSDOLine(CO_Data *d, UNS8 line)
{
    ...
    MSG_DEBUG("objdict->line, index: 0x%X, subindex: 0x%X", 
            d->transfers[line].index, d->transfers[line].subIndex);
...
UNS8 lineToSDO (CO_Data *d, UNS8 line, UNS32 nbBytes, UNS8 *data)
{
    ...
    {
        MSG_ERR("SDO Size of data %d too large. Exceed SDO_MAX_LENGTH_TRANSFER", nbBytes);
        return 0xFF;
    }
#endif
    ...
    {
        MSG_ERR("SDO Size of data %d too large. Exceed count", nbBytes);
        return 0xFF;
    }
    ...
    else
    {
        if (d->transfers[line].dynamicData == NULL)
        {
            MSG_ERR("SDO's dynamic buffer not allocated. Line %d", line);
            return 0xFF;
        }
        for (i = 0 ; i < nbBytes ; i++)
        {
...
UNS8 SDOtoLine(CO_Data *d, UNS8 line, UNS32 nbBytes, UNS8 *data)
{
    ...
    if (d->transfers[line].dataType == domain)
    {
        ...
        {
            MSG_WAR("Domain func is invalid");
        }
    }
    else
    {
        ...
        if (requiredSize > SDO_MAX_LENGTH_TRANSFER)
        {
            if (d->transfers[line].dynamicData == NULL)
            {
                ...
                if (d->transfers[line].dynamicData == NULL)
                {
                    MSG_ERR("SDO allocating dynamic buffer failed, size %d", d->transfers[line].dynamicDataSize);
                    return 0xFF;
                }
                /* Copy present data */
            ...
            else if (requiredSize > d->transfers[line].dynamicDataSize)
            {
                ...
                {
                    MSG_ERR("SDO reallocating dynamic buffer failed, size %d", newDynamicDataSize);
                    return 0xFF;
                }
        ...
        UNS8 subIndex, UNS32 abortCode)
{
    ...
    { 
        MSG_DEBUG("FailedSDO : line %d found", line);
    }

    if ((!err) && (whoami == SDO_SERVER))
    {
        resetSDOLine( d, line );
        MSG_DEBUG("FailedSDO : line %d released", line);
    }
    ...
    MSG_DEBUG("Sending SDO abort ");
    err = sendSDOAbort(d, whoami, CliServNbr, index, subIndex, abortCode);
    if (err)
    {
        MSG_ERR("Unable to send the SDO abort");
        return 0xFF;
    }
    return 0;
}
...
{
    MSG_DEBUG("reset SDO line %d", line);
    initSDOLine(d, line, 0, 0, 0, SDO_RESET);
    for (UNS32 i = 0; i < SDO_MAX_LENGTH_TRANSFER; i++)
    {
...
{
    MSG_DEBUG("init SDO line %d", line);
    if (line > SDO_MAX_SIMULTANEOUS_TRANSFERS)
    {
        MSG_ERR("SDO line %d is invalid", line);
        return 0xFF;
    }
    ...
    d->transfers[line].dynamicData = NULL;
...
UNS8 getSDOFreeLine(CO_Data *d, UNS8 whoami, UNS8 *line)
{
    ...
    MSG_ERR("Too many SDO in progress (max %d). Aborted.", SDO_MAX_SIMULTANEOUS_TRANSFERS);
    return 0xFF;
}
...
UNS8 closeSDOTransfer(CO_Data *d, UNS8 nodeId, UNS8 whoami)
{
    ...
    if (err)
    {
        MSG_ERR("No SDO communication to close");
        return 0xFF;
    }
    resetSDOLine(d, line);
    return 0;
...
UNS8 sendSDO(CO_Data *d, UNS8 whoami, UNS8 CliServNbr, UNS8 *pData)
{
    ...
    {
        MSG_DEBUG("Unable to send the SDO (not in op or pre-op mode), %d", d->nodeState);
        return 0xFF;
    }
    ...
    if (whoami == SDO_SERVER)
    {
        ...
        {
            MSG_ERR("SendSDO : SDO server not found");
            return 0xFF;
        }
        m.cob_id = UNS16_LE( (UNS16) READ_UNS32(d->objdict, offset+CliServNbr, 2) );
        MSG_DEBUG("I am server Tx cobId : %d", m.cob_id);
    }
    else /*case client*/
    { 
        /* Get the client->server cobid.*/
        ...
        {
            MSG_ERR("SendSDO : SDO client not found");
            return 0xFF;
        }
        m.cob_id = UNS16_LE( (UNS16) READ_UNS32(d->objdict, offset+CliServNbr, 1) );
        MSG_DEBUG("I am client Tx cobId : %d", m.cob_id);
    }
...
UNS8 sendSDOAbort(CO_Data *d, UNS8 whoami, UNS8 CliServNbr, UNS16 index, UNS8 subIndex, UNS32 abortCode)
{
    ...
    MSG_DEBUG("Sending SDO abort 0x%X", abortCode);
    data[0] = 0x80;
    /* Index */
...
static UNS8 getWhoAmI(CO_Data *d, UNS32 msgCobId, UNS8 *CliServNbrPtr, UNS8 *nodeIdPtr)
{
    ...
    UNS16 lastIndex = d->lastIndex->SDO_SVR;

    // Search SDO server firstly
    if(currentIndex)
    {
        while (currentIndex <= lastIndex)
        {
            UNS16 numSRV = currentIndex - d->firstIndex->SDO_SVR;
            if (d->objdict[currentIndex].bSubCount <= 1)
            {
                MSG_ERR("Subindex 1 not found at index 0x%X", 0x1200 + numSRV);
                return 0xFF;
            }

            // Looking for the cobid received.
            if (READ_UNS32(d->objdict, currentIndex, 1) == msgCobId)
            {
                whoami = SDO_SERVER;
                MSG_DEBUG("proceedSDO. I am server. index : 0x%X", 0x1200 + numSRV);
                // Defining Server number = index minus 0x1200 where the cobid received is defined.
                *CliServNbrPtr = numSRV;
                break;
            }
    ...
    // Search SDO client if do not find cobid in SDO servers
    if (whoami == SDO_UNKNOWN)
    {
        // Am-I client ?
        currentIndex = d->firstIndex->SDO_CLT;
        lastIndex = d->lastIndex->SDO_CLT;
        if(currentIndex)
        {
            while (currentIndex <= lastIndex)
            {
                UNS16 numCLT = currentIndex - d->firstIndex->SDO_CLT;
                if (d->objdict[currentIndex].bSubCount <= 3)
                {
                    MSG_ERR("Subindex 3 not found at index 0x%X", 0x1280 + numCLT);
                    return 0xFF;
                }

                // Looking for the cobid received.
                if (READ_UNS32(d->objdict, currentIndex, 2) == msgCobId)
                {
                    whoami = SDO_CLIENT;
                    MSG_DEBUG("proceedSDO. I am client index : 0x%X", 0x1280 + numCLT);
                    // Defining Client number = index minus 0x1280 where the cobid received is defined.
                    *CliServNbrPtr = numCLT;
                    // Reading the server node ID, if client it is mandatory in the OD
                    *nodeIdPtr = READ_UNS8(d->objdict, currentIndex, 3);
                    break;
                }
...
static UNS8 serverInitDomainDownload(CO_Data *d, UNS8 line, UNS8 CliServNbr, UNS8 *data_field)
{
    ...
    MSG_DEBUG("Received SDO Initiate Download (to store data) defined at index 0x%X", 0x1200+CliServNbr);
    MSG_DEBUG("Writing at index : 0x%X", index);
    MSG_DEBUG("Writing at subIndex : 0x%X", subIndex);

    // Check if a SDO transfer has already been initiated
    if (line < SDO_MAX_SIMULTANEOUS_TRANSFERS)
    {
        MSG_ERR("SDO error : Transmission already started.");
        failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, SDOABT_LOCAL_CTRL_ERROR);
        return 0xFF;
    }

    // No line on use. Great !  Try to open a new line.
    UNS8 ret = getSDOFreeLine(d, SDO_SERVER, &line);
    if (ret)
    {
        MSG_ERR("SDO error : No line free, too many SDO in progress. Aborted.");
        failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, SDOABT_LOCAL_CTRL_ERROR);
        return 0xFF;
    }
    ...
    if (getSDOe(data_field[0])) // If SDO expedited, only need trans one time
    {
        /* nb of data to be downloaded */
        UNS32 nbBytes = 4 - getSDOn2(data_field[0]);
        /* Storing the data in the line structure. */
        d->transfers[line].count = nbBytes;
        ret = SDOtoLine(d, line, nbBytes, data_field+4);
        if (ret)
        {
        ...
        MSG_DEBUG("SDO Initiate Download is an expedited transfer. Finished. ");
        ...
        if (errorCode)
        {
            MSG_ERR("SDO error : Unable to copy the data in the object dictionary");
    ...
    else // It is not an expedited transfer, need trans multi times
    {
        if (getSDOs(data_field[0]))
        {
            UNS32 nbBytes = (data_field[4]) + ((UNS32)(data_field[5])<<8) + ((UNS32)(data_field[6])<<16) + ((UNS32)(data_field[7])<<24);
            ret = setSDOLineRestBytes(d, line, nbBytes);
            if (ret)
            {
                failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, SDOABT_GENERAL_ERROR);
                return 0xFF;
    ...
    resp_data[0] = 3 << 5;
    resp_data[1] = (UNS8)(index & 0xFF);        /* LSB */
    resp_data[2] = (UNS8)((index >> 8) & 0xFF); /* MSB */
    resp_data[3] = subIndex;
    for (UNS8 i = 4 ; i < 8 ; i++)
        resp_data[i] = 0;
    sendSDO(d, SDO_SERVER, CliServNbr, resp_data);
...
static UNS8 serverDownloadDomainSegment(CO_Data *d, UNS8 line, UNS8 CliServNbr, UNS8 *data_field)
{
    ...
    {
        MSG_ERR("SDO error : Received download segment for unstarted trans. index 0x%X",
                0x1200+CliServNbr);
    ...
    MSG_DEBUG("Received SDO download segment defined at index 0x%X", 0x1200+CliServNbr);
    UNS16 index = d->transfers[line].index;
    UNS8 subIndex = d->transfers[line].subIndex;
    /* Check Toggle bit. */
    if (d->transfers[line].toggle != getSDOt(data_field[0]))
    {
        MSG_ERR("SDO error : Toggle error : %d", getSDOt(data_field[0]));
        failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, SDOABT_TOGGLE_NOT_ALTERNED);
        return 0xFF;
    }
    /* Nb of data to be downloaded */
    UNS32 nbBytes = 7 - getSDOn3(data_field[0]);
    /* Store the data in the transfer structure. */
    UNS8 ret = SDOtoLine(d, line, nbBytes, data_field + 1);
    if (ret)
    {
        failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, SDOABT_GENERAL_ERROR);
        return 0xFF;
    ...
    MSG_DEBUG("SDO. Send response to download request defined at index 0x%x", 0x1200 + CliServNbr);
    sendSDO(d, SDO_SERVER, CliServNbr, resp_data);
    ...
    if (getSDOc(data_field[0]))
    {
        ...
        /* Release of the line */
        resetSDOLine(d, line);
        MSG_DEBUG("SDO. End of download defined at index 0x1200 + %d", CliServNbr);
    }
...
static UNS8 serverInitDomainUpload(CO_Data *d, UNS8 line, UNS8 CliServNbr, UNS8 *data_field)
{
    UNS16 index = (UNS16)getSDOindex(data_field[1], data_field[2]);
    UNS8 subIndex = getSDOsubIndex(data_field[3]);
    MSG_DEBUG("Received SDO Initiate upload (to send data) defined at index 0x1200 + %d", CliServNbr);
    MSG_DEBUG("Reading at index : 0x%X", index);
    MSG_DEBUG("Reading at subIndex : 0x%X", subIndex);
    
    // Check if a SDO transfer has already yet initiated
    if (line < SDO_MAX_SIMULTANEOUS_TRANSFERS)
    {
        MSG_ERR("SDO error : Transmission already started at line %d", line);
        MSG_ERR("Server Nbr = %d", CliServNbr);
        failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, SDOABT_LOCAL_CTRL_ERROR);
        return 0xFF;
    }
    ...
    /* error code while reading or writing in the local object dictionary. */
    UNS32 errorCode = objDictToSDOLine(d, line);
    if (errorCode)
    {
        MSG_ERR("SDO error : Unable to copy the data from object dictionary. Err code : 0x%X",
                errorCode);
        failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, errorCode);
        return 0xFF;
    }
    ...
    if (nbBytes > 4)
    {
        ...
        resp_data[0] = (UNS8)((2 << 5) | 1);
        resp_data[1] = (UNS8)(index & 0xFF);        /* LSB */
        resp_data[2] = (UNS8)((index >> 8) & 0xFF); /* MSB */
        resp_data[3] = subIndex;
        resp_data[4] = (UNS8) nbBytes;
        resp_data[5] = (UNS8) (nbBytes >> 8);
        resp_data[6] = (UNS8) (nbBytes >> 16);
        resp_data[7] = (UNS8) (nbBytes >> 24);
        MSG_DEBUG("SDO. Sending normal upload initiate response defined at index 0x%X", 0x1200 + nodeId);
        sendSDO(d, SDO_SERVER, CliServNbr, resp_data);
    }
    else
    {
        /* Expedited upload. (cs = 2 ; e = 1) */
        resp_data[0] = (UNS8)((2 << 5) | ((4 - nbBytes) << 2) | 3);
        resp_data[1] = (UNS8)(index & 0xFF);        /* LSB */
        resp_data[2] = (UNS8)((index >> 8) & 0xFF); /* MSB */
        resp_data[3] = subIndex;
        ret = lineToSDO(d, line, nbBytes, resp_data + 4);
        if (ret)
        {
            failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, SDOABT_GENERAL_ERROR);
            return 0xFF;
        ...
        for (UNS8 i = 4 + nbBytes ; i < 8 ; i++)
            resp_data[i] = 0;
        MSG_DEBUG("SDO. Sending expedited upload initiate response defined at index 0x1200 + %d", CliServNbr);
        sendSDO(d, SDO_SERVER, CliServNbr, resp_data);
        // Release the line.
        resetSDOLine(d, line);
    }
...
static UNS8 serverUploadDomainSegment(CO_Data *d, UNS8 line, UNS8 CliServNbr, UNS8 *data_field)
{
    ...
    {
        MSG_ERR("SDO error : Received upload segment for unstarted trans. index 0x1200 + %d", CliServNbr);
        failedSDO(d, CliServNbr, SDO_SERVER, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
        return 0xFF;
    }
    ...
    // Reset the wathdog
    RestartSDO_TIMER(d, line);
    MSG_DEBUG("Received SDO upload segment defined at index 0x1200 + %d", CliServNbr);
    UNS16 index = d->transfers[line].index;
    UNS8 subIndex = d->transfers[line].subIndex;
    ...
    if (nbBytes > 7)
    {
        ...
        resp_data[0] = (UNS8)(d->transfers[line].toggle << 4);
        UNS8 ret = lineToSDO(d, line, 7, resp_data + 1);
        if (ret)
        {
            failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, SDOABT_GENERAL_ERROR);
            return 0xFF;
        ...
        d->transfers[line].toggle = (UNS8)(! (d->transfers[line].toggle & 1));
        MSG_DEBUG("SDO. Sending upload segment defined at index %X", 0x1200 + CliServNbr);
        sendSDO(d, SDO_SERVER, CliServNbr, resp_data);
    }
    else
    {
        ...
        resp_data[0] = (UNS8)((d->transfers[line].toggle << 4) | ((7 - nbBytes) << 1) | 1);
        UNS8 ret = lineToSDO(d, line, nbBytes, resp_data + 1);
        if (ret)
        {
            failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, SDOABT_GENERAL_ERROR);
            return 0xFF;
        }
        for (UNS8 i = nbBytes + 1 ; i < 8 ; i++)
            resp_data[i] = 0;
        MSG_DEBUG("SDO. Sending last upload segment defined at index %X", 0x1200 + CliServNbr);
        sendSDO(d, SDO_SERVER, CliServNbr, resp_data);
        // Release the line
        resetSDOLine(d, line);
    }
...
static UNS8 serverBlockUpload(CO_Data *d, UNS8 line, UNS8 CliServNbr, UNS8 *data_field)
{
    ...
    if (SubCommand == SDO_BCS_INITIATE_UPLOAD_REQUEST)
    {
        UNS16 index = (UNS16)getSDOindex(data_field[1], data_field[2]);
        UNS8 subIndex = getSDOsubIndex(data_field[3]);
        MSG_DEBUG("Received SDO Initiate block upload defined at index %X", 0x1200 + CliServNbr);
        MSG_DEBUG("Reading at index : 0x%X", index);
        MSG_DEBUG("Reading at subIndex : 0x%X", subIndex);

        // Check if a SDO transfer have already been initiated
        if (line < SDO_MAX_SIMULTANEOUS_TRANSFERS)
        {
            MSG_ERR("SDO error : Transmission already started at line %d", line);
            MSG_ERR("Server Nbr = %d", CliServNbr);
        ...
        if(data_field[4] > 127)
        {
            MSG_ERR("SDO error : invalid block size");
            failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, SDOABT_INVALID_BLOCK_SIZE);
            return 0xFF;
        }
        ...
        d->transfers[line].peerCRCsupport = (UNS8)(((data_field[0])>>2) & 1);
        d->transfers[line].blksize = data_field[4];
        /* Transfer data from dictionary to the line structure. */
        /* error code while reading or writing in the local object dictionary. */
        UNS32 errorCode = objDictToSDOLine(d, line);
        if (errorCode)
        ...
        MSG_DEBUG("SDO. Sending normal block upload initiate response defined at index 0x%X", 0x1200 + nodeId);
        sendSDO(d, SDO_SERVER, CliServNbr, data);
    }
    else if (SubCommand == SDO_BCS_END_UPLOAD_REQUEST)
    {
        MSG_DEBUG("Received SDO block END upload request defined at index 0x1200 + %d", CliServNbr);
        // A SDO transfer should have already been initiated.
        if ( (line >= SDO_MAX_SIMULTANEOUS_TRANSFERS) ||
             (d->transfers[line].state != SDO_BLOCK_UPLOAD_IN_PROGRESS) )
        {
            MSG_ERR("SDO error : Received block upload request for unstarted trans. index 0x1200 + %d", CliServNbr);
            failedSDO(d, CliServNbr, SDO_SERVER, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
            return 0xFF;
        }
        // Release the line
    ...
    else if ((SubCommand == SDO_BCS_UPLOAD_RESPONSE) || (SubCommand == SDO_BCS_START_UPLOAD))
    {
        ...
        if ( (line >= SDO_MAX_SIMULTANEOUS_TRANSFERS) ||
             (d->transfers[line].state != SDO_BLOCK_UPLOAD_IN_PROGRESS) )
        {
            MSG_ERR("SDO error : Received block upload response for unstarted trans. index 0x1200 + %d", CliServNbr);
            failedSDO(d, CliServNbr, SDO_SERVER, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
            return 0xFF;
        }
        // Reset the wathdog
        ...
        {
            MSG_DEBUG("Received SDO block upload response defined at index 0x1200 + %d", CliServNbr);
            d->transfers[line].blksize = data_field[2];
            // Sequence number of last segment that was received successfully
            UNS8 AckSeq = (data_field[1]) & 0x7f;
            UNS32 nbBytes = 0;
            getSDOLineRestBytes(d, line, &nbBytes);
            ...
            if((nbBytes == 0) && (AckSeq == d->transfers[line].seqno))
            {
                ...
                for (UNS8 i = 1; i < 8; i++)
                    data[i] = 0;
                MSG_DEBUG("SDO. Sending block END upload response defined at index 0x1200 + %d", CliServNbr);
                sendSDO(d, SDO_SERVER, CliServNbr, data);
                return 0;
            }
            else
            ...
            {
                MSG_ERR("SDO error : Received upload response with bad ackseq index 0x1200 + %d", CliServNbr);
                failedSDO(d, CliServNbr, SDO_SERVER, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
                return 0xFF;
            }
        }
        ...
        {
            MSG_DEBUG("Received SDO block START upload defined at index 0x1200 + %d", CliServNbr);
        }

        d->transfers[line].lastblockoffset = d->transfers[line].offset;
        for(UNS8 SeqNo = 1; SeqNo <= d->transfers[line].blksize; SeqNo++)
        {
            ...
            if (nbBytes > 7)
            {
                // The segment to transfer is not the last one
                data[0] = SeqNo;
                UNS8 ret = lineToSDO(d, line, 7, data + 1);
                if (ret)
                {
                    failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, SDOABT_GENERAL_ERROR);
                    return 0xFF;
                }
                MSG_DEBUG("SDO. Sending upload segment defined at index 0x1200 + %d", CliServNbr);
                sendSDO(d, SDO_SERVER, CliServNbr, data);
            }
            else
            {
                ...
                data[0] = 0x80 | SeqNo;
                UNS8 ret = lineToSDO(d, line, nbBytes, data + 1);
                if (ret)
                {
                    failedSDO(d, CliServNbr, SDO_SERVER, index, subIndex, SDOABT_GENERAL_ERROR);
                    return 0xFF;
                ...
                for (UNS8 i = nbBytes + 1; i < 8; i++)
                    data[i] = 0;
                MSG_DEBUG("SDO. Sending last upload segment defined at index 0x1200 + %d", CliServNbr);
                sendSDO(d, SDO_SERVER, CliServNbr, data);
...
static UNS8 serverBlockDownload(CO_Data *d, UNS8 line, UNS8 CliServNbr, UNS8 *data_field, UNS8 nodeId)
{
    ...
    if (line >= SDO_MAX_SIMULTANEOUS_TRANSFERS)
    {
        // Nothing already started, no SDO trans initiated
        UNS8 SubCommand = (data_field[0]) & 1;
        if (SubCommand != SDO_BCS_INITIATE_DOWNLOAD_REQUEST)
        {
            MSG_ERR("SDO error block download : Received wrong subcommand from node id %d", nodeId);
            failedSDO(d, CliServNbr, SDO_SERVER, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
            return 0xFF;
        }
        UNS16 index = getSDOindex(data_field[1], data_field[2]);
        UNS8 subIndex = getSDOsubIndex(data_field[3]);
        MSG_DEBUG("Received SDO block download initiate defined at index 0x1200 + %d", CliServNbr);
        MSG_DEBUG("Writing at index : 0x%X", index);
        MSG_DEBUG("Writing at subIndex: 0x%X", subIndex);
        /* Try to open a new line. */
        UNS8 ret = getSDOFreeLine( d, SDO_SERVER, &line );
        ...
        d->transfers[line].peerCRCsupport = (UNS8)(((data_field[0])>>2) & 1);
        if (data_field[0] & 2)    /* if data set size is indicated */
            d->transfers[line].objsize = (UNS32)data_field[4] + (UNS32)data_field[5]*256 + (UNS32)data_field[6]*256*256 + (UNS32)data_field[7]*256*256*256;
        ...
        MSG_DEBUG("SDO. Sending block download initiate response - index 0x1200 + %d", CliServNbr);
        sendSDO(d, SDO_SERVER, CliServNbr, data);
    }
    else if (d->transfers[line].rxstep == RXSTEP_STARTED)
    {
        MSG_DEBUG("Received SDO block download data segment - index 0x1200 + %d", CliServNbr);
        RestartSDO_TIMER(d, line);
        UNS8 SeqNo = data_field[0] & 0x7F;
        if (data_field[0] & 0x80) /* Last segment ? */
        {
            if(SeqNo == (d->transfers[line].seqno + 1))
            {
                ...
                memcpy(d->transfers[line].tmpData, data_field, 8);
            }
            ...
            MSG_DEBUG("SDO. Sending block download response - index 0x1200 + %d", CliServNbr);
            sendSDO(d, SDO_SERVER, CliServNbr, data);
            d->transfers[line].seqno = 0;
        }
        else
        {
            ...
            {    
                d->transfers[line].seqno = SeqNo;
                /* Store the data in the transfer structure. */
                UNS8 ret = SDOtoLine(d, line, 7, data_field + 1);
                if (ret)
                {
    ...
    {
        MSG_DEBUG("Received SDO block download end request - index 0x1200 + %d", CliServNbr);
        /* here store remaining bytes in tmpData to line, check size and confirm or abort */
        if ((data_field[0] & 1) != SDO_BCS_END_DOWNLOAD_REQUEST)
        {
            MSG_ERR("SDO error block download : Received wrong subcommand - index 0x1200 + %d", CliServNbr);
            failedSDO(d, CliServNbr, SDO_SERVER, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
            return 0xFF;
        }
        ...
        /* Store the data in the transfer structure. */
        UNS8 ret = SDOtoLine(d, line, 7-NbBytesNoData, d->transfers[line].tmpData + 1);
        if (ret)
        {
            failedSDO(d, CliServNbr, SDO_SERVER, d->transfers[line].index,  d->transfers[line].subIndex, SDOABT_GENERAL_ERROR);
            return 0xFF;
        ...
        { 
            ...
            {
                MSG_ERR("SDO error block download : sizes do not match - index 0x1200 + %d", CliServNbr);
                failedSDO(d, CliServNbr, SDO_SERVER, d->transfers[line].index, d->transfers[line].subIndex, SDOABT_LOCAL_CTRL_ERROR);
                return 0xFF;
            }
        }
        ...
        for (UNS8 i = 1; i < 8; i++)
            data[i] = 0;
        MSG_DEBUG("SDO. Sending block download end response - index 0x1200 + %d", CliServNbr);
        sendSDO(d, SDO_SERVER, CliServNbr, data);
        ...
        /* Release of the line */
        resetSDOLine(d, line);
        MSG_DEBUG("SDO. End of block download defined at index 0x1200 + %d", CliServNbr);
    }
...
static UNS8 clientUploadDomainSegment(CO_Data *d, UNS8 line, UNS8 CliServNbr, UNS8 *data_field, UNS8 nodeId)
{
    if ( (line >= SDO_MAX_SIMULTANEOUS_TRANSFERS) ||
         (d->transfers[line].state != SDO_UPLOAD_IN_PROGRESS) )
    {
        MSG_ERR("SDO error : Received segment response for unknown trans. from nodeId %d", nodeId);
        failedSDO(d, CliServNbr, SDO_CLIENT, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
        return 0xFF;
    }
    ...
    if (d->transfers[line].toggle != getSDOt(data_field[0]))
    {
        MSG_ERR("SDO error : Received segment response Toggle error. from nodeId %d", nodeId);
        failedSDO(d, CliServNbr, SDO_CLIENT, index, subIndex, SDOABT_TOGGLE_NOT_ALTERNED);
        return 0xFF;
    }
    ...
    if (ret)
    {
        failedSDO(d, CliServNbr, SDO_CLIENT, index, subIndex, SDOABT_GENERAL_ERROR);
        return 0xFF;
    }
    ...
    if (getSDOc(data_field[0]))
    {
        ...
        MSG_DEBUG("SDO. End of upload from node : %d", nodeId);
    }
    else // more segments to receive
    {
        ...
        for (UNS8 i = 1; i < 8; i++)
            data[i] = 0;
        sendSDO(d, SDO_CLIENT, CliServNbr, data);
        MSG_DEBUG("SDO send upload segment request to nodeId %d", nodeId);
    }
...
static UNS8 clientDownloadDomainSegment(CO_Data *d, UNS8 line, UNS8 CliServNbr, UNS8 *data_field, UNS8 nodeId)
{
    ...
    if (nbBytes == 0)
    {
        MSG_DEBUG("SDO End download. segment response received. OK. from nodeId %d", nodeId);
        StopSDO_TIMER(d, line);
        d->transfers[line].state = SDO_FINISHED;
    ...
    MSG_DEBUG("SDO sending download segment to nodeId %d", nodeId);
    sendSDO(d, SDO_CLIENT, CliServNbr, data);
...
static UNS8 clientInitiateDomainUpload(CO_Data *d, UNS8 line, UNS8 CliServNbr, UNS8 *data_field, UNS8 nodeId)
{
    if ( (line >= SDO_MAX_SIMULTANEOUS_TRANSFERS) ||
         (d->transfers[line].state != SDO_UPLOAD_IN_PROGRESS) )
    {
        MSG_ERR("SDO error : Received response for unknown upload request from nodeId %d", nodeId);
        failedSDO(d, CliServNbr, SDO_CLIENT, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
        return 0xFF;
    }
    /* Reset the wathdog */
    ...
    if (getSDOe(data_field[0])) /* If SDO expedited */
    {
        /* nb of data to be uploaded */
        UNS32 nbBytes = 4 - getSDOn2(data_field[0]);
        /* Storing the data in the line structure. */
        UNS8 ret = SDOtoLine(d, line, nbBytes, data_field + 4);
        if (ret)
        {
            failedSDO(d, CliServNbr, SDO_CLIENT, index, subIndex, SDOABT_GENERAL_ERROR);
            return 0xFF;
        ...
        MSG_DEBUG("SDO expedited upload finished. Response received from node : %d", nodeId);
        StopSDO_TIMER(d, line);
        d->transfers[line].count = nbBytes;
        d->transfers[line].state = SDO_FINISHED;
        if(d->transfers[line].Callback)
    ...
    else /* So, if it is not an expedited transfer */
    {
        ...
        {
            UNS32 nbBytes = data_field[4] + ((UNS32)(data_field[5])<<8) + ((UNS32)(data_field[6])<<16) + ((UNS32)(data_field[7])<<24);
            UNS8 ret = setSDOLineRestBytes(d, line, nbBytes);
            if (ret)
            {
                failedSDO(d, CliServNbr, SDO_CLIENT, index, subIndex, SDOABT_GENERAL_ERROR);
                return 0xFF;
        ...
        for (UNS8 i = 1; i < 8; i++)
            data[i] = 0;
        MSG_DEBUG("SDO. Sending upload segment request to node : %d", nodeId);
        sendSDO(d, SDO_CLIENT, CliServNbr, data);
    }
...
static UNS8 clientInitiateDomainDownload(CO_Data *d, UNS8 line, UNS8 CliServNbr, UNS8 *data_field, UNS8 nodeId)
{
    if ( (line >= SDO_MAX_SIMULTANEOUS_TRANSFERS) ||
         (d->transfers[line].state != SDO_DOWNLOAD_IN_PROGRESS) )
    {
        MSG_ERR("SDO error : Received response for unknown download request from nodeId %d", nodeId);
        failedSDO(d, CliServNbr, SDO_CLIENT, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
        return 0xFF;
    }
    ...
    if (nbBytes == 0)
    {
        MSG_DEBUG("SDO End download expedited. Response received. from nodeId %d", nodeId);
        StopSDO_TIMER(d, line);
        d->transfers[line].state = SDO_FINISHED;
        if(d->transfers[line].Callback)
        {
...
static UNS8 clientBlockDownload(CO_Data *d, UNS8 line, UNS8 CliServNbr, UNS8 *data_field, UNS8 nodeId)
{
    ...
    if ((SubCommand == SDO_BSS_INITIATE_DOWNLOAD_RESPONSE) || (SubCommand == SDO_BSS_DOWNLOAD_RESPONSE))
    {
        ...
        if ( (line >= SDO_MAX_SIMULTANEOUS_TRANSFERS) ||
             (d->transfers[line].state != SDO_BLOCK_DOWNLOAD_IN_PROGRESS))
        {
            MSG_ERR("SDO error : Received response for unknown block download request from node id %d", nodeId);
            failedSDO(d, CliServNbr, SDO_CLIENT, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
            return 0xFF;
        }
        UNS16 index = d->transfers[line].index;
        ...
        if (SubCommand == SDO_BSS_INITIATE_DOWNLOAD_RESPONSE)
        {
            ...
            {
                MSG_ERR("SDO error : invalid block size");
                failedSDO(d, CliServNbr, SDO_CLIENT, index, subIndex, SDOABT_INVALID_BLOCK_SIZE);
                return 0xFF;
            }
            d->transfers[line].peerCRCsupport = ((data_field[0])>>2) & 1;
            d->transfers[line].blksize = data_field[4];
        }
        else
        {
            ...
            if((nbBytes == 0) && (AckSeq == d->transfers[line].seqno))
            {
                ...
                for (UNS8 i = 1 ; i < 8 ; i++)
                    data[i] = 0;
                MSG_DEBUG(0x3AA5, "SDO. Sending block END download request defined at index 0x1200 + %d", CliServNbr);
                sendSDO(d, SDO_CLIENT, CliServNbr, data);
                return 0;
            }
            else
            ...
            if(d->transfers[line].offset > d->transfers[line].count) /* Bad AckSeq reveived (too high) */
            {
                MSG_ERR("SDO error : Received upload segment with bad ackseq index 0x1200 + %d", CliServNbr);
                failedSDO(d, CliServNbr, SDO_CLIENT, index, subIndex, SDOABT_LOCAL_CTRL_ERROR);
                return 0xFF;
            }
        }
        ...
        for(UNS8 SeqNo = 1; SeqNo <= d->transfers[line].blksize; SeqNo++)
        {
            ...
            if (nbBytes > 7)
            {
                ...
                MSG_DEBUG("SDO. Sending download segment to node id %d", nodeId);
                sendSDO(d, SDO_CLIENT, CliServNbr, data);
            }
            else
            {
                ...
                for (UNS8 i = nbBytes + 1 ; i < 8 ; i++)
                    data[i] = 0;
                MSG_DEBUG("SDO. Sending last download segment to node id %d", nodeId);
                sendSDO(d, SDO_CLIENT, CliServNbr, data);
                d->transfers[line].endfield = (UNS8) (7 - nbBytes);
                return 0;
            }
    ...
    {
        MSG_DEBUG("SDO End block download response from nodeId %d", nodeId);
        StopSDO_TIMER(d, line);
        d->transfers[line].state = SDO_FINISHED;
        if(d->transfers[line].Callback)
        {
    ...
    {
        MSG_ERR("SDO error block download : Received wrong subcommand from nodeId %d", nodeId);
        failedSDO(d, CliServNbr, SDO_CLIENT, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
        return 0xFF;
    }
...
static UNS8 clientBlockUpload(CO_Data *d, UNS8 line, UNS8 CliServNbr, UNS8 *data_field, UNS8 nodeId)
{
    if (line >= SDO_MAX_SIMULTANEOUS_TRANSFERS)
    {
        ...
        MSG_ERR("SDO error block upload : no transmission started (Node %d)", nodeId);
        failedSDO(d, CliServNbr, SDO_CLIENT, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
        return 0xFF;
    }
    ...
    {
        if ((data_field[0] & 1) == SDO_BSS_INITIATE_UPLOAD_RESPONSE)
        {
            MSG_DEBUG("Received SDO block upload response from node id %d", nodeId);
            d->transfers[line].rxstep = RXSTEP_STARTED;
            d->transfers[line].peerCRCsupport = (UNS8)(((data_field[0])>>2) & 1);
            if ((data_field[0]) & 2)    // if data set size is indicated
                d->transfers[line].objsize = (UNS32)data_field[4] + (UNS32)data_field[5]*256 + (UNS32)data_field[6]*256*256 + (UNS32)data_field[7]*256*256*256;
            ...
            for (UNS8 i = 1 ; i < 8 ; i++)
                data[i] = 0;
            MSG_DEBUG("SDO. Sending block upload start to node id %d", nodeId);
            sendSDO(d, SDO_CLIENT, CliServNbr, data);
        }
    }
    else if (d->transfers[line].rxstep == RXSTEP_STARTED)
    {
        ...
        if (data_field[0] & 0x80) // Last segment ?
        {
            ...
            data[2] = SDO_BLOCK_SIZE;
            data[3] = data[4] = data[5] = data[6] = data[7] = 0;
            MSG_DEBUG("SDO. Sending block upload response to node id %d", nodeId);
            sendSDO(d, SDO_CLIENT, CliServNbr, data);
            d->transfers[line].seqno = 0;
        }
        else
        {
            if (SeqNo == (d->transfers[line].seqno + 1))
            {
                ...
                if (ret)
                {
                    failedSDO(d, CliServNbr, SDO_CLIENT, d->transfers[line].index,  d->transfers[line].subIndex, SDOABT_GENERAL_ERROR);
                    return 0xFF;
                }
            }
            if (SeqNo == SDO_BLOCK_SIZE)
    ...
    {
        // here store remaining bytes in tmpData to line, check size and confirm or abort
        if ((data_field[0] & 1) != SDO_BSS_END_UPLOAD_RESPONSE)
        {
            MSG_ERR("SDO error block upload : Received wrong subcommand from node id %d", nodeId);
            failedSDO(d, CliServNbr, SDO_CLIENT, 0, 0, SDOABT_LOCAL_CTRL_ERROR);
            return 0xFF;
        }
        // Number of bytes that do not contain data in last segment of block transfer
        ...
        { 
            if (d->transfers[line].objsize != d->transfers[line].offset)
            {
                MSG_ERR("SDO error block download : sizes do not match - from node id %d", nodeId);
                failedSDO(d, CliServNbr, SDO_CLIENT, d->transfers[line].index, d->transfers[line].subIndex, SDOABT_LOCAL_CTRL_ERROR);
                return 0xFF;
            }
        }
        ...
        MSG_DEBUG("SDO. Sending block upload end request to node id %d", nodeId);
        sendSDO(d, SDO_CLIENT, CliServNbr, data);
        MSG_DEBUG("SDO. End of block upload request");
        StopSDO_TIMER(d, line);
        d->transfers[line].state = SDO_FINISHED;
        if(d->transfers[line].Callback)
        {
...
UNS8 proceedSDO(CO_Data *d, Message *m)
{
    ...
    UNS32 msgCobId = UNS16_LE(m->cob_id);
    UNS8 commandByte = m->data[0];
    MSG_DEBUG("proceedSDO");

    /* Looking for the cobId in the object dictionary. */
    /* Am-I a server ? */
    whoami = getWhoAmI(d, msgCobId, &CliServNbr, &nodeId);
    if (whoami == SDO_CLIENT)
    {
        MSG_DEBUG("I am CLIENT number %d", CliServNbr);
    }
    else if (whoami == SDO_SERVER)
    {
        MSG_DEBUG("I am SERVER number %d", CliServNbr);
    }
    else
    {
        MSG_ERR("Unknown SERVER/CLIENT");
    ...
    if (!ret) // line on use
    {
        ...
        {        
            if(commandByte == 0x80)    /* If first byte is 0x80 it is an abort frame (seqno = 0 not allowed) */
                cs = 4;
            else
                cs = 6;
        }
    ...
    {
        line = SDO_MAX_SIMULTANEOUS_TRANSFERS; // assign invalid value, means line not used
    }

    /* Other cases : cs is specified */
    if (cs == 0xFF)
    {
        cs = getSDOcs(commandByte);
    }
    ...
    if (whoami == SDO_SERVER)
    {
        if (cs == 0)
        {
            /* Receiving a download segment data : an SDO transfer should have already been initiated. */
            ret = serverDownloadDomainSegment(d, line, CliServNbr, m->data);
        }
        else if (cs == 1)
        {
            /* Receive of an initiate download */
            ret = serverInitDomainDownload(d, line, CliServNbr, m->data);
        }
        else if (cs == 2)
        {
            /* Receive of an initiate upload.*/
            ret = serverInitDomainUpload(d, line, CliServNbr, m->data);
        }
        else if (cs == 3)
        {
            /* Receiving a upload segment. */
            /* A SDO transfer should have been yet initiated. */
            ret = serverUploadDomainSegment(d, line, CliServNbr, m->data);
        }
        else if (cs == 4)
        {
            /* Received SDO abort. */
            ...
            {
                resetSDOLine(d, line);
                MSG_WAR("SD0. Received SDO abort. Line released. Code : 0x%X", abortCode);
            }
            else
            {
                MSG_WAR("SD0. Received SDO abort. No line found. Code : 0x%X", abortCode);
            }
        ...
        {
            ret = serverBlockUpload(d, line, CliServNbr, m->data);
        }
        else if (cs == 6)
        {
            ret = serverBlockDownload(d, line, CliServNbr, m->data, nodeId);
        }
    }
    else // Client
    {
        if (cs == 0)
        {
            ...
            ret = clientUploadDomainSegment(d, line, CliServNbr, m->data, nodeId);
        }
        else if (cs == 1)
        {
            /* It is a response for a previous download segment. We should find a line opened for this. */
            ret = clientDownloadDomainSegment(d, line, CliServNbr, m->data, nodeId);
        }
        else if (cs == 2)
        {
            /* It is the response for the previous initiate upload request.*/
            /* We should find a line opened for this. */
            ret = clientInitiateDomainUpload(d, line, CliServNbr, m->data, nodeId);
        }
        else if (cs == 3)
        {
            /* It is the response for the previous initiate download request. */
            /* We should find a line opened for this. */
            ret = clientInitiateDomainDownload(d, line, CliServNbr, m->data, nodeId);
        }
        else if (cs == 4)
        {
            /* Received SDO abort. */
            ...
            if (line < SDO_MAX_SIMULTANEOUS_TRANSFERS)
            {
                ...
                d->transfers[line].abortCode = abortCode;
                MSG_WAR("SD0. Received SDO abort. Line state ABORTED. Code : 0x%X", abortCode);
                if(d->transfers[line].Callback) (*d->transfers[line].Callback)(d,nodeId);
            }
            else
            {
        ...
        {
            ret = clientBlockDownload(d, line, CliServNbr, m->data, nodeId);
        }
        else if (cs == 6)
        {
            ret = clientBlockUpload(d, line, CliServNbr, m->data, nodeId);
        }
    }

    if (cs > 6) // Error : Unknown cs
    {
        if (line < SDO_MAX_SIMULTANEOUS_TRANSFERS)
        ...
        MSG_ERR("SDO. Received unknown command specifier : 0x%X", cs);
        ret = 0xFF;
    }
...
UNS8 getSDOClientFromNodeId( CO_Data *d, UNS8 nodeId )
{
    ...
    if (offset == 0)
    {
        MSG_ERR("No SDO client index found for nodeId %d", nodeId);
        return 0xFF;
    }
    ...
    while (offset <= lastIndex)
    {
        ...
        nodeIdServer = READ_UNS8(d->objdict, offset, 3);
        MSG_DEBUG("index : 0x%X", 0x1280 + CliNbr);
        MSG_DEBUG("nodeIdServer : %d", nodeIdServer);
    ...
    if (!SDOfound)
    {
        MSG_WAR("SDO No preset client found to communicate with node : %d", nodeId);
        return 0xFE;
    }
    MSG_DEBUG("SDO client defined at index : 0x%X", 0x1280 + CliNbr);
        ...
        UNS8 subIndex, UNS32 count, UNS8 dataType, void *data, SDOCallback_t Callback, UNS8 endianize, UNS8 useBlockMode)
{
    ...
    MSG_DEBUG("Send SDO to write in the dictionary of node : %d", nodeId);
    MSG_DEBUG("                                   At index : 0x%X", index);
    MSG_DEBUG("                                   subIndex : 0x%X", subIndex);
    MSG_DEBUG("                                   nb bytes : %d", count);

    /* Check that the data can fit in the transfer buffer */
#ifndef SDO_DYNAMIC_BUFFER_ALLOCATION
    if(count > SDO_MAX_LENGTH_TRANSFER)
    {
        MSG_ERR("SDO error : request for more than SDO_MAX_LENGTH_TRANSFER bytes to transfer to node : %d", nodeId);
        return 0xFF;
    }
#endif
    ...
    if (!err)
    {
        MSG_ERR("SDO error : Communication yet established. with node : %d", nodeId);
        return 0xFF;
    }
    /* Taking the line ... */
    ...
    if (err)
    {
        MSG_ERR("SDO error : No line free, too many SDO in progress. Aborted for node : 0x%X", nodeId);
        return (0xFF);
    }
    else
    {
        MSG_DEBUG("Transmission on line : %d", line);
    }
...
#ifdef SDO_DYNAMIC_BUFFER_ALLOCATION
    {
        ...
        if (count > SDO_MAX_LENGTH_TRANSFER)
        {
            ...
            if (d->transfers[line].dynamicData == NULL)
            {
                MSG_ERR("SDO. Error. Could not allocate enough bytes : %d", count);
                resetSDOLine(d, line);
                return 0xFE;
            }
            lineData = d->transfers[line].dynamicData;
...
#  endif
    ...
    if (err)
    {
        MSG_ERR("SDO. Error while sending SDO to node : %d", nodeId);
        /* release the line */
        resetSDOLine(d, line);
        return 0xFF;
    }
        ...
        UNS8 subIndex, UNS32 count, UNS8 dataType, void *data, SDOCallback_t Callback, UNS8 endianize, UNS8 useBlockMode)
{
    ...
    if(ret == 0xFE)
    {
        ...
        if (offset == 0)
        {
            MSG_ERR("writeNetworkDict : No SDO client index found");
            return 0xFF;
        }
        i = 0;
        while (offset <= lastIndex)
        {
            if (d->objdict[offset].bSubCount <= 3)
            {
                MSG_ERR("Subindex 3 not found at index 0x%X", 0x1280 + i);
                return 0xFF;
            }
            nodeIdServer = READ_UNS8(d->objdict, offset, 3);
            if(nodeIdServer == 0)
            {
                WRITE_UNS32(d->objdict, offset, 1, (UNS32)(0x600 + nodeId));
                WRITE_UNS32(d->objdict, offset, 2, (UNS32)(0x580 + nodeId));
                WRITE_UNS8(d->objdict, offset, 3, nodeId);
...
UNS8 _readNetworkDict (CO_Data *d, UNS8 nodeId, UNS16 index, UNS8 subIndex, UNS8 dataType, SDOCallback_t Callback, UNS8 useBlockMode)
{
    ...
    MSG_DEBUG("Send SDO to read in the dictionary of node : %d", nodeId);
    MSG_DEBUG("                                  At index : 0x%X", index);
    MSG_DEBUG("                                  subIndex : 0x%X", subIndex);
    ...
    if (err)
    {
        MSG_ERR("SDO error : No line free, too many SDO in progress. Aborted for node : %d", nodeId);
        return (0xFF);
    }
    else
    {
        ...
        UNS32 *abortCode)
{
    ...
    * abortCode = 0;
    ...
    if (err)
    {
        MSG_ERR("SDO error : No line found for communication with node : %d", nodeId);
        *size = 0;
        return SDO_ABORTED_INTERNAL;
    }
