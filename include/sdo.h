 ...
 *  SDOs are always confirmed point-to-point communication services.
 *  @ingroup comobj
 */
 ...
 * - set to RXSTEP_STARTED when the client receives the initiate upload response
 * - set to RXSTEP_END when the last segment of a block is received
 */
...
typedef struct {
  ...
  UNS32          offset;     //< Stack pointer of data[]
                             // Used only to transfer part of a line to or from an SDO.
  ...
  UNS8           peerCRCsupport;    //< True if the peer supports CRC
  UNS8           blksize;           //< Number of segments per block with 0 < blksize < 128
  UNS8           ackseq;            //< Sequence number of the last segment received successfully
  ...
  UNS8           endfield;          //< Number of bytes in the last segment of the last block that do not contain data
  rxStep_t       rxstep;            //< Data consumer receive step
  ...
  TIMER_HANDLE   timer;      //< Time counter to implement a timeout in milliseconds.
                             ...
                             // SDO_UPLOAD_IN_PROGRESS, and reset to 0
...
/** 
 * @brief Copy data to an existing line from the argument "* data"
 * @param d Pointer on a CAN object data structure
 * @param line SDO line
 * @param nbBytes
 * @param data Pointer on the data
 ...
 * because much information is stored there: index, subindex, data received or transmitted, ...
...
/** 
 * @brief Transmit an SDO error to the client. The reasons may be:
 ...
 *        Must be called after getRead/WriteResultNetworkDict in case of error.
 * @param d
 * @param nodeId
 */
...
/** 
 * @ingroup sdo
 * @brief Used to send an SDO request frame to write the data at the indicated index and subIndex
 * @param d Pointer to a CAN object data structure
 * @param nodeId Node Id of the slave
 * @param index At index indicated
 * @param subIndex At subIndex indicated
 * @param count Number of bytes to write in the dictionary.
 ...
 * end of the exchange. (on success or abort).
 * @param d Pointer to a CAN object data structure
 * @param nodeId Node Id of the slave
 * @param index At index indicated
 * @param subIndex At subIndex indicated
...
/**
 * @ingroup sdo 
 * @brief Used to send an SDO request frame to read.
 * @param d Pointer to a CAN object data structure
 * @param nodeId Node Id of the slave
 * @param index At index indicated
 * @param subIndex At subIndex indicated
 ...
 * @param abortCode Pointer to the abort code. (0 = not available. Else: SDO abort code received if return SDO_ABORTED_RCV)
 * 
 * 
 * @return
...
/**
 * @ingroup sdo
 * @brief Use this function after calling writeNetworkDict to get the result of the write.
 * @details It is mandatory to call this function because it releases the line used for the transfer.
 * @param d Pointer to a CAN object data structure
 * @param nodeId Node Id of the slave
 * @param abortCode Pointer to the abort code
 * - 0 = not available. 
