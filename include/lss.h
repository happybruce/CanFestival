 ...
 * @brief LSS offers the possibility to inquire and change settings of certain local-layer parameters on
...
struct struct_lss_transfer {
    UNS8          state;      /* State of the transmission: takes values LSS_... */
    UNS8          command;    /* The LSS command of the transmission */
    UNS8          mode;       /* LSS mode */
    
    UNS32         dat1;       /* the data from the last msg received */
    UNS8          dat2; 
    
    UNS8 nodeID;              /* New node ID stored to apply when switching to LSS operational */
    ...
    TIMER_HANDLE timerMSG;    /* timerMSG is automatically incremented whenever 
                                * the lss state is in LSS_TRANS_IN_PROGRESS, and reset to 0
                                * when the response LSS have been received.
                                */
                                
    TIMER_HANDLE timerSDELAY; /* timerSDELAY is automatically incremented whenever 
                                * the lss switchDelayState is in SDELAY_FIRST or SDELAY_SECOND, and reset to 0
    ...
    TIMER_HANDLE timerFS;     /* timerFS is automatically incremented when the FastScan service
                                * has been requested and reset to 0 when the protocol ends. */
 ...
 * end of the exchange (on success or abort) and can be NULL.
 * @return sendLSS(d,command,dat1,dat2)
